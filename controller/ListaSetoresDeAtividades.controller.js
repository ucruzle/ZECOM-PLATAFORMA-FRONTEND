sap.ui.define([
	...
	return Controller.extend("sap.ecom.plataforma.controller.ListaSetoresDeAtividades", {
		...
		onAtualizarConsulta: function() {
			...
			var oModel  = null;
			var SetoresDeAtividades = [];
			var SetorDeAtividade  = {};
			...
			SetoresDeAtividades.push(SetorDeAtividade);
			...
			var oData = {
				SetoresDeAtividades: SetoresDeAtividades,
				Status: [],
				navigation: [
					{
						title: 'Início',
		...
		onbtnCriarSetorDeAtividade: function() {
			this._showSetorDeAtividadeFragment();
		},

		onEditDetalheSetorDeAtividade: function(oEvent) {
			this._showSetorDeAtividadeFragment();
		},

		onGravarSetorDeAtividade: function(oEvent) {
			this.onCloseSetorDeAtividade();
		},

		onCloseSetorDeAtividade: function() {
			this._activeScreen = "view";
			this._SetorDeAtividadeFragment.close();
		},

		_showSetorDeAtividadeFragment: function() {

			if (!this._SetorDeAtividadeFragment) {
				this._SetorDeAtividadeFragment = sap.ui.xmlfragment("SetorDeAtividadeFragment",
					"sap.ecom.plataforma.view.ListaSetoresDeAtividades.fragments.GeraSetorDeAtividade", 
					this
				);
				this.getView().addDependent(this._SetorDeAtividadeFragment);
			}
			this._SetorDeAtividadeFragment.open();
		},
